...
const TransferFunds =  ({ btnRef, walletData, banks, email }: { btnRef: any, banks: any[], email: string, walletData: { accountName: string, accountNumber: string } }) => {
  ...
  return (
    <div className="drawer drawer-end w-9 ml-auto z-10">
      ...
      <div>
        <label ref={btnRef} htmlFor="transferFunds-drawer" className="btn btn-circle btn-sm btn-outline glass drawer-button text-gray-950 align-middle hidden">
        </label>
      </div>
      <div className="drawer-side z-30">
        ...
        <div className="menu p-4 w-[350px] lg:w-[400px] min-h-full flex flex-col text-base-content rounded-xl bg-base">
          ...
          <section className="h-auto w-full grow mt-16">
            <TransferFundsForm banks={banks} email={email} />
          </section>
        </div>
      </div>
    </div>
