...
import { editOffer } from '@/app/lib/actions';
import { useEffect, useRef, useState } from 'react';
import clsx from 'clsx';
import type { Offer } from '@prisma/client';

export default function EditOfferForm({ offer }: { offer: Offer | null }) {
  const initialState = {
    message: {
      severity: "",
      message: "",
  ...
  const editOfferWithUserId = editOffer.bind(null, offer?.id);
  const [state, dispatch] = useFormState(editOfferWithUserId, initialState);
  ...
  return (
    <form action={dispatch} className="mt-10">
      ...
      <label className="form-control w-full mb-2" htmlFor='offerCategory'>
        ...
        <select className="select select-bordered rounded-lg w-full" defaultValue={offer?.offerCategory} onChange={(e) => setOfferCategory(e.target.value)} id='offerCategory' name='offerCategory'>
      ...
      <label className="form-control w-full mb-2" htmlFor='cardName'>
        ...
        <select className="select select-bordered rounded-lg w-full"  defaultValue={offer?.cardName} id='cardName' name='cardName'>
      ...
      {offerCategory === "seller" ?
        <label className="form-control w-full mb-2" htmlFor='valueInUSD'>
          ...
          <input type="number" placeholder=""  defaultValue={offer?.valueInUSD || ""} className="input input-bordered rounded-lg w-full text-sm" id='valueInUSD' name='valueInUSD' />
        ...
        <label className={"form-control w-full mb-2"} htmlFor='minAmount'>
          ...
          <div className="w-full flex items-center">
            <input type="number" placeholder="Min Value in $" defaultValue={offer?.minAmount || ""} className="input input-bordered rounded-lg w-1/2 mr-1 text-sm" id='minAmount' name='minAmount' />
            <input type="number" placeholder="Max Value in $" defaultValue={offer?.maxAmount || ""} className="input input-bordered rounded-lg w-1/2 ml-1 text-sm" id='maxAmount' name='maxAmount' />
          </div>
      ...
      <label className="form-control w-full mb-2" htmlFor='rate'>
        ...
        <input type="number" placeholder="" defaultValue={offer?.rate || ""} className="input input-bordered rounded-lg w-full text-sm" id='rate' name='rate' />
      ...
      <label className="form-control w-full mb-2" htmlFor='cardType'>
        ...
        <select className="select select-bordered rounded-lg w-full" defaultValue={offer?.cardType || ""} id='cardType' name='cardType'>
...
function EditOfferButton() {
  ...
  return (
    <Button className="mt-10 w-full glass" aria-disabled={pending}>
      Save <ArrowRightIcon className="ml-auto h-5 w-5 text-sm text-gray-50" />
    </Button>
  );
}
